...
angular.module('mean.users').factory('MeanUser', [ '$rootScope', '$http', '$location', '$stateParams', '$cookies', '$q', '$timeout',
  function($rootScope, $http, $location, $stateParams, $cookies, $q, $timeout) {
    ...
    MeanUserKlass.prototype.registerCompany = function(user) {
      $http.post('/api/registerCompany', {
        email: user.email,
    ...
    MeanUserKlass.prototype.checkCompanyAdmin = function(companyId) 
    {
      ...
      $http.get('/api/loggedin').success(function(user) 
      {
        ...
        if (user !== '0')
        {
          ...
          {
            if(user.companies[i].id == companyId)
            {
              for(var j = 0; j < user.companies[i].roles.length; j++)
              {
                if(user.companies[i].roles[j] === 'Company_Admin')
                {
