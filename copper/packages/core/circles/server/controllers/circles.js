...
module.exports = function(Circles, app) {
    ...
    return {
        ...
        mineFromCompany: function(req, res) 
        {
            var descendants = {};
            for (var index in req.acl.company.circles) 
            {
                descendants[index] = req.acl.company.circles[index].decendants;
            }
            return res.send({allowed: req.acl.company.allowed, descendants: descendants });
        },
        ...
        allFromCompany: function(req, res) {
            return res.send({
                tree:req.acl.company.tree,
                circles: req.acl.company.circles
            });
        },
        ...
        companyAcl: function(req, res, next) {
            // console.log('companyAcl');
            ...
            roles.forEach(function(role) 
            {
                ...
                {    
                    ...
                    companyRoles[role] = req.acl.circles[role];
                }
            });

            var tree = Circle.buildTrees(companyRoles);
        ...
        hasCompanyCircle: function(circle) 
        {
            return function (req, res, next) 
            {
                if(!req.user || req.acl.company.allowed.indexOf(circle) === -1) 
                {
                    return res.status(403).send('User is not authorized for this action');
                }
                next();
        ...
        hasCompanyCircleBoolean: function(req, circle) 
        {
            console.log('hasCompanyCircleBoolean');
            console.log(circle);
            console.log(req.acl.company.allowed);
            if(!req.user || req.acl.company.allowed.indexOf(circle) === -1) 
            {
                return false;
            }     
