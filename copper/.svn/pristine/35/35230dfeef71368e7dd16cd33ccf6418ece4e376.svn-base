 ...
 * Basic Role checking - future release with full permission system
 */
exports.requiresCompanyAdmin = function(req, res, next) 
{
  if (!req.isAuthenticated()) 
  {
    return res.status(401).send('User is not authorized');
  }
  console.log('requiresCompanyAdmin');
  ...
  findUser(req.user._id, function(user) 
  {
      ...
      {
        if (user.companies[i].id == req.params.company) 
        {
          if (user.companies[i].roles.indexOf('Company_Admin') === -1) 
          {
            return res.status(401).send('User is not authorized');
          }
        } 
